| [index](../README.md) | new-run-all | [run-s](run-s.md) | [run-p](run-p.md) | [Node API](node-api.md) |
|-----------------------|-------------|-------------------|-------------------|-------------------------|
...
On the other hand, this `new-run-all` command runs multiple scripts in parallel or sequential.

### Run scripts sequentially

```
$ new-run-all clean lint build
```
...
```
$ new-run-all --parallel lint build
```
...
**Note2:** `&` operator does not work on Windows' `cmd.exe`. But `new-run-all --parallel` works fine there.

### Run a mix of sequential and parallel scripts

```
$ new-run-all clean lint --parallel watch:html watch:js
```
...
```
$ new-run-all a b --parallel c d --sequential e f --parallel g h i
```
or

```
$ new-run-all a b --parallel c d --serial e f --parallel g h i
```
...
`new-run-all` reads the actual npm-script list from `package.json` in the current directory, then filters the scripts by glob-like patterns, then runs those.
...
```
$ new-run-all --parallel "build:* -- --watch"
...
```
$ new-run-all build "start-server -- --port {1}" -- 8080
```
...
{
    "scripts": {
        "start": "new-run-all build \"start-server -- --port {1}\" --"
    }
}
```
...
- If `--print-label` option is given, some tools in scripts might stop coloring their output.
  ...
  `new-run-all` changes the `process.stdout` of child processes to a pipe in order to add labels to the head of each line if `--print-label` option is given.<br>
  For example, [eslint] stops coloring under `new-run-all --print-label`. But [eslint] has `--color` option to force coloring, we can use it. For anything [chalk] based you can set the environment variable `FORCE_COLOR=1` to produce colored output anyway.
